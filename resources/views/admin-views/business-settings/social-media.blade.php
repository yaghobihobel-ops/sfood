                ...
                <div class="col-sm mb-2 mb-sm-0">
                    <h1 class="page-header-title text-capitalize">
                        <div class="card-header-icon d-inline-flex mr-2 img">
                            <img src="{{dynamicAsset('/public/assets/admin/img/social.png')}}" alt="public">
                        </div>
                        <span>
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <label for="link" class="ml-1">{{ translate('messages.social_media_link')}}
                                            <span class="input-label-secondary text--title" data-toggle="tooltip" data-placement="right" data-original-title='{{translate("The_configured_social_link_from_here_will_be_visible_in_the_footer_section_of_the_website.")}}'>
                        ...
                        for (var count = 0; count < data.length; count++) {
                            ...
                            html += `<td class="column_name" data-column_name="status" data-id="${data[count].id}">
                                <label class="toggle-switch toggle-switch-sm" for="${data[count].id}">
                                    <input type="checkbox" class="toggle-switch-input status"
                                    id='${data[count].id}' ${data[count].status == 1 ? "checked" : ""}
                                    data-id="${data[count].id}"
                                    data-type="status"
                                    data-image-on="{{ dynamicAsset('/public/assets/admin/img/modal/${data[count].name}-on.png') }}"
                                    data-image-off="{{ dynamicAsset('/public/assets/admin/img/modal/${data[count].name}-off.png') }}"
                                    data-title-on="<strong>${data[count].name} {{translate('is_Enabled!')}}"
                                    data-title-off="<strong> ${data[count].name} {{translate('is_Disabled!')}}"
                                    data-text-on="<p>{{ translate('If_enabled,_restaurants_have_to_provide_collected_cash_by_them_self') }}</p>"
                                    data-text-off="<p>${data[count].name} {{translate('is_disabled_now_no_one_can_use_or_see_this_Social_Medial')}}</p>"
                                        >
