    ...
    <div class="content container-fluid">
        ...
        <div class="tab-content">
            <div class="tab-pane fade show active">
                <div class="card mb-3">
                    @php($mail_status=\App\Models\BusinessSetting::where('key','subscription_deadline_mail_status_restaurant')->first()?->value ?? '0')
                    ...
                    <div class="card-body">
                        ...
                        <span class="form-label-secondary text--primary" data-toggle="tooltip" data-placement="right" data-original-title="{{ translate('Subscribed_restaurants_will_get_a_reminder_mail_when_their_subscription_is_about_to_end.') }}">
                            ...
                            <label class="toggle-switch toggle-switch-sm">
                                ...
                                <input type="checkbox" class="status toggle-switch-input dynamic-checkbox"
                                       ...
                                       data-text-on="<p>{{translate('If_enabled,Subscribed_restaurants_will_get_a_reminder_mail_when_their_subscription_is_about_to_end.')}}</p>"
                                       data-text-off="<p>{{translate('If_disabled,Subscribed_restaurants_will_not_get_a_reminder_mail_when_their_subscription_is_about_to_end.')}}</p>"
                        ...
                        <form action="{{route('admin.business-settings.email-status',['restaurant','subscription-deadline',$mail_status == '1'?0:1])}}" method="get" id="mail-status_form">
                        </form>
                    </div>
                </div>
                @php($data=\App\Models\EmailTemplate::where('type','restaurant')->where('email_type', 'subscription-deadline')->first())
                @php($template= $template ?? $data?->email_template ?? 5)
                <form action="{{ route('admin.business-settings.email-setup', ['restaurant','subscription-deadline']) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                            ...
                            <div class="email-format-wrapper">
                                ...
                                <div class="right-content">
                                    <div class="d-flex flex-wrap justify-content-between __gap-15px mt-2 mb-5">
                                        @php($data=\App\Models\EmailTemplate::withoutGlobalScope('translate')->where('type','restaurant')->where('email_type', 'subscription-deadline')->first())
