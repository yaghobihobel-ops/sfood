                            ...
                            <div class="row g-2" id="additional_Image_Section">
                                @if(!empty($delivery_man->identity_image))
                                    @foreach($delivery_man['identity_image_full_url'] as $index => $identityImage)
                                        @if(!empty($identityImage))
                                            @php
                                                $fileName = basename(parse_url($identityImage, PHP_URL_PATH));
                                            @endphp
                                            <div class="col-sm-6 col-md-4 col-lg-3 col-xxl-2">
                                                <div class="custom_upload_input custom_upload_preview position-relative bg-white rounded border-dashed-2 text-center" data-filename="{{ $fileName }}">
                                                    <input type="hidden" class="existing-image-name" value="{{ $fileName }}">
                                                    <input type="file" name="identity_image[]" class="custom-upload-input-file action-add-more-image"
                                                           data-index="{{ $index + 1 }}" data-imgpreview="additional_Image_{{ $index + 1 }}"
                                                           accept=".jpg, .png, .webp, .jpeg, .gif, .bmp, .tif, .tiff|image/*"
                                                           data-target-section="#additional_Image_Section"
                                                    >
                                                    <span class="delete_file_input delete_file_input_section btn btn-outline-danger btn-sm square-btn d-flex" onclick="markImageDeleted(this)">
                                                        <i class="tio-delete"></i>
                                                    </span>
                                                    <div class="overlay">
                                                    ...
                                                    <div class="img_area_with_preview p-0 z-index-2">
                                                        <img id="additional_Image_{{ $index + 1 }}" class="bg-white"
                                                             src="{{ $identityImage }}" alt="{{translate('Identity Image')}}">
                                                    </div>
                                                    <div class="position-absolute h-100 top-0 w-100 d-flex align-content-center p-2 justify-content-center">
                                                        <div class="d-flex flex-column justify-content-center align-items-center">
                                                            <img alt="" width="30"
                                                                 src="{{ dynamicAsset('public/assets/admin/img/doc-uploaded.png') }}">
                                                            <div class="text-title mt-2 fs-12 text-wrap flex-wrap">
                                                                {{translate('Select a file or')}} <span class="font-semibold text-title">{{translate('Drag & Drop')}}</span>
                                                                {{translate('here')}}</div>
                                                        </div>
                                                    </div>
                                                </div>
                                ...
                                <div class="col-sm-6 col-md-4 col-lg-3 col-xxl-2">
                                    <div class="custom_upload_input custom_upload_preview position-relative bg-white rounded border-dashed-2 text-center">
                                        @php
                                            $nextIndex = (is_array($delivery_man['identity_image_full_url']) ? count($delivery_man['identity_image_full_url']) : 0) + 1;
                                        @endphp
                                        <input type="file" name="identity_image[]" class="custom-upload-input-file action-add-more-image"
                                               data-index="{{ $nextIndex }}" data-imgpreview="additional_Image_{{ $nextIndex }}"
                                               accept=".jpg, .png, .webp, .jpeg, .gif, .bmp, .tif, .tiff|image/*"
                                               data-target-section="#additional_Image_Section"
                                        >
                                        <span class="delete_file_input delete_file_input_section btn btn-outline-danger btn-sm square-btn d-none" onclick="markImageDeleted(this)">
                                            <i class="tio-delete"></i>
                                        </span>
                                        <div class="overlay">
                                            <div class="d-flex gap-1 justify-content-center align-items-center h-100">
                                        ...
                                        <div class="img_area_with_preview p-0 z-index-2">
                                            <img id="additional_Image_{{ $nextIndex }}" class="bg-white d-none"
                                                 src="" alt="">
                                        </div>
                                        <div
                                            ...
                                            <div
                                                ...
                                                <img alt="" width="30"
                                                     src="{{ dynamicAsset('public/assets/admin/img/doc-uploaded.png') }}">
                                                <div class="text-title mt-2 fs-12 text-wrap flex-wrap">{{translate('Select a file or')}} <span class="font-semibold text-title">{{translate('Drag & Drop')}}</span>
                                                    {{translate('here')}}</div>
                                            </div>
                                        </div>
                                    </div>
                    ...
                    <div class="card-body">
                        <div class="row g-3">
                                                ...
                                                @if ($item['media_data'] != null)
                                                    ...
                                                    <div class="col-md-4 col-12 image_count_{{ $key }}" data-id="{{ $key }}" >
                                                        <div class="form-group m-0">
                                                            ...
                                                            <input id="{{ $item['input_data'] }}" type="{{ $item['field_type'] }}" name="additional_documents[{{ $item['input_data'] }}][]" class="form-control h--45px"
                                                                   placeholder="{{ translate($item['placeholder_data']) }}"
                            ...
                            <div class="col-lg-12">
                                <div class="p-xxl-20 p-12 global-bg-box rounded">
                                    <div class="mb-20">
                                        <h5 class="mb-1">{{ translate('Driving license') }}</h5>
                                        <p class="mb-0 fs-12">{{ translate('pdf, doc, jpg. File size : max 2 MB') }}</p>
                                    </div>
                                    <div class="row g-3">
                                        @foreach($delivery_man['additional_documents_full_url'] as $img)
                                            <div class="col-6 col-sm-4 col-md-3 col-lg-3 col-xl-2">
                                                <img class="rounded w-100" width='170' height="150" src="{{ $img }}" alt="{{ translate('Driving license') }}">
                                            </div>
                                        @endforeach
                                    </div>
                                </div>
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-sm-6 col-lg-4">
                                ...
                                <div class="js-form-message form-group m-0">
                                    <label class="input-label" for="signupSrPassword">{{translate('messages.password')}} <span class="text-danger">*</span>
                            ...
                            <div class="col-sm-6 col-lg-4">
                                ...
                                <div class="js-form-message form-group m-0">
                                    <label class="input-label" for="signupSrConfirmPassword">{{translate('messages.confirm_password')}} <span class="text-danger">*</span>
                                    </label>
...
@push('script_2')
    ...
    <script>
        ...
        function addMoreImage(thisData, targetSection) {
            // Only consider "new" upload slots (not existing images) when deciding to add another
            ...
            // Enforce max count: existing + newly selected
            ...
            if (totalCount >= IDENTITY_MAX) {
                // Remove any empty new slots if limit reached
                $newSlotContainers.each(function(){
                    const $inp = $(this).find("input[type='file']");
                    if ($inp.length && $inp.prop('files').length === 0) {
        ...
        // On load: if already at max, remove empty uploader
        (function(){
    ...
    <script>
        function markImageDeleted(btn) {
            var $container = $(btn).closest('.custom_upload_input');
            var $col = $container.closest('[class*="col-"]');
            var existingName = $container.find('.existing-image-name').val();
            // If existing image, append hidden input for deletion
            if (existingName) {
                if ($('form#deliveryman-form input[name="deleted_images[]"][value="' + existingName + '"]').length === 0) {
                    $('<input>', {
                        type: 'hidden',
                        name: 'deleted_images[]',
                        value: existingName
            ...
            // After deletion: if under max and no empty new slot exists, append one
            const targetSection = '#additional_Image_Section';
            const totalExisting = $(targetSection + ' .existing-image-name').length;
            const totalNewSelected = $(targetSection + " input[type='file'][name='identity_image[]']").filter(function(){
                return this.files && this.files.length > 0;
            ...
            let emptyNewSlots = $(targetSection + " .custom_upload_input").filter(function(){
                return $(this).find('.existing-image-name').length === 0 && $(this).find("input[type='file']").length && $(this).find("input[type='file']").prop('files').length === 0;
            }).length;

            if (totalCount < IDENTITY_MAX && emptyNewSlots === 0) {
                // Determine next unique dataset index
                let maxIndex = 0;
                $(targetSection + " input[type='file'][name='identity_image[]']").each(function(){
                    const idx = parseInt(this.dataset.index || '0', 10);
                    if (!isNaN(idx) && idx > maxIndex) maxIndex = idx;
                });
                let datasetIndex = maxIndex + 1;
                ...
                let newHtmlData = `<div class="col-sm-6 col-md-4 col-lg-3 col-xxl-2">
                                <div class="custom_upload_input custom_upload_preview  position-relative bg-white border-dashed-2">
                                    <input type="file" name="identity_image[]" class="custom-upload-input-file action-add-more-image" data-index="${datasetIndex}" data-imgpreview="additional_Image_${datasetIndex}"
                                        accept=".jpg, .webp, .png, .jpeg, .gif, .bmp, .tif, .tiff|image/*" data-target-section="${targetSection}">
