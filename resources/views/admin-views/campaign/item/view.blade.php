                ...
                <div class="row align-items-md-center">
                    <div class="col-md-6 col-lg-4 mb-3 mb-md-0">
                        <img class="rounded initial-16 onerror-image" id="viewer"
                            ...
                            data-onerror-image="{{ dynamicAsset('public/assets/admin/img/900x400/img1.png') }}"
                            alt="image">
                    </div>
                    <div class="col-md-6">
                            ...
                            @if ($campaign->restaurant)
                                <a href="{{ route('admin.restaurant.view', $campaign->restaurant_id) }}" class="d-block">
                                    <img class="avatar-img avatar-circle initial-17 onerror-image" id="viewer"
                                        ...
                                        data-onerror-image="{{ dynamicAsset('/public/assets/admin/img/100x100/restaurant-default-image.png') }}"
                                        alt="image">
                                    ...
                                    <tr>
                                        ...
                                        <td class="px-4">
                                            <div>
                                                ...
                                                <span class="d-block text-dark">{{ translate('messages.price') }} :
                                                    <strong>{{ \App\CentralLogics\Helpers::format_currency($campaign['price']) }}</strong>
                                        ...
                                        @if ($productWiseTax)
                                            <td>
                                                <span class="d-block font-size-sm text-body">
                                                    @forelse ($campaign?->taxVats?->pluck('tax.name', 'tax.tax_rate')->toArray() as $key => $tax)
                    ...
                    <form>
                        <div class="input--group input-group">
                            <input type="text" name="search" value="{{ request()?->search ?? null }}"
                                class="form-control" placeholder="{{ translate('Search_here_by_restaurants') }}">
                            ...
                            @foreach ($orders as $key => $order)
                                <tr class="status-{{ $order['order_status'] }} class-all">
                                    ...
                                    <td class="table-column-pl-0">
                                        <a
                                            href="{{ route('admin.order.details', ['id' => $order->order_id]) }}">{{ $order->order['id'] }}</a>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <label
                                            class="badge badge-soft-primary">{{ Str::limit($order->order->restaurant ? $order->order->restaurant->name : translate('messages.Restaurant_deleted!'), 20, '...') }}</label>
                                    </td>
                                    <td>
