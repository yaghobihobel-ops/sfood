            ...
            <div class="d-flex flex-wrap align-items-center gap-2">
                ...
                <div class="w-16rem">
                    <select name="delivery_man_id"
                            ...
                            data-url="{{url()->full()}}" data-filter="delivery_man_id">
                ...
                <div class="w-16rem">
                    <select name="payment_method_id"
                            ...
                            class="js-select2-custom form-control set-filter"
                            data-url="{{url()->full()}}" data-filter="payment_method_id">
            ...
            <div class="search--button-wrapper">
                ...
                <div class="hs-unfold ml-3">
                    ...
                    <div id="usersExportDropdown"
                        ...
                        <a id="export-excel" class="dropdown-item" href="{{route('admin.dm-disbursement.export', ['id'=>$disbursement->id,'type'=>'excel',request()->getQueryString()])}}">
                        ...
                        <a id="export-csv" class="dropdown-item" href="{{route('admin.dm-disbursement.export', ['id'=>$disbursement->id,'type'=>'csv',request()->getQueryString()])}}">
                        ...
                        <a id="export-pdf" class="dropdown-item" href="{{route('admin.dm-disbursement.export', ['id'=>$disbursement->id,'type'=>'pdf',request()->getQueryString()])}}">
                    ...
                    <tbody>
                        @foreach($disbursement_delivery_mans as $key => $dm)
                            <tr>
                                ...
                                <td>
                                    <div>
                                        {{$dm?->withdraw_method?->method_name ?? translate('Default_method')}}
                                    </div>
                                </td>
                                <td>
                                ...
                                <td>
                                    <div class="btn--container justify-content-center">
                                        ...
                                        @if($dm->status == 'completed')
                                            <a class="btn btn-sm btn--danger btn-outline-danger action-btn action-btn-section" href="{{route('admin.dm-disbursement.change-status', ['id'=>$dm->id,'status'=>'pending'])}}" data-toggle="tooltip" title="{{ translate('revert') }}">
                                                <i class="tio-restore"></i>
                                            </a>
                                        @else
                                            @if($dm->status != 'canceled')
                                                <button class="btn btn-sm btn--danger btn-outline-danger action-btn action-btn-section form-alert-cancel" data-url="{{route('admin.dm-disbursement.change-status', ['id'=>$dm->id,'status'=>'canceled'])}}" data-toggle="tooltip" title="{{ translate('cancel') }}">
                                                    <i class="tio-clear"></i>
                                                </button>
                                            @endif
                                            <a class="btn btn-sm btn--primary btn-outline-primary action-btn action-btn-section" href="{{route('admin.dm-disbursement.change-status', ['id'=>$dm->id,'status'=>'completed'])}}" title="{{ translate('complete') }}" data-toggle="tooltip">
                                                <i class="tio-done"></i>
                                            </a>
                                        @endif
                                    </div>
                                        ...
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-body">
                                                            ...
                                                            <div class="item w-100">
                                                                ...
                                                                <ul class="item-list">
                                                                    <li class="d-flex flex-wrap">
                                                                        ...
                                                                        <strong>{{$dm?->withdraw_method?->method_name ?? translate('Default_method')}}</strong>
                                                                    </li>
                                                                    <li class="d-flex flex-wrap">
                                                ...
                                                <div class="mt-3 btn--container justify-content-end">
                                                    @if($dm->status =='pending')
                                                        <a type="button" href="{{route('admin.dm-disbursement.change-status', ['id'=>$dm->id,'status'=>'canceled'])}}" class="btn btn--reset" >{{ translate('cancel') }}</a>
                                                    @endif
                                                    @if($dm->status != 'completed')
                                                        <a type="button" href="{{route('admin.dm-disbursement.change-status', ['id'=>$dm->id,'status'=>'completed'])}}" class="btn btn--primary">{{ translate('complete') }}</a>
                                                    @endif
                                                </div>
                                            </div>
                                        </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $('#complete').on('click', function() {
                $.get({
                    url: '{{ route('admin.dm-disbursement.status') }}',
                    dataType: 'json',
                    data: {
