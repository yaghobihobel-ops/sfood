...
//                'react_header_status'=>(isset($settings['react_header_status']) )  ? $settings['react_header_status'] : null ,
    ...
    private function getDefaultPaymentMethods()
    {
        ...
        $methods = DB::table('addon_settings')->where('is_active',1)->whereIn('settings_type', ['payment_config'])->whereIn('key_name', ['ssl_commerz','paypal','stripe','razor_pay','senang_pay','paytabs','paystack','paymob_accept','paytm','flutterwave','liqpay','bkash','mercadopago','saman','pasargad'])->get();
        $env = env('APP_ENV') == 'live' ? 'live' : 'test';
        $credentials = $env . '_values';
