...
class PaystackController extends Controller
{
    ...
    public function handleGatewayCallback(Request $request)
    {
        ...
        if ($paymentDetails['status'] == true) {
            $this->payment::where(['id' => $paymentDetails['data']['metadata']['orderID']])->update([
                'payment_method' => 'paystack',
                'is_paid' => 1,
                'transaction_id' => $request['trxref'],
            ]);
            $data = $this->payment::where(['id' => $paymentDetails['data']['metadata']['orderID']])->first();
        ...
        $payment_data = $this->payment::where(['id' => $paymentDetails['data']['metadata']['orderID']])->first();
