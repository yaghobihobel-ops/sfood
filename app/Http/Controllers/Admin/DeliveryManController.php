...
class DeliveryManController extends Controller
{
    ...
    public function store(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            'identity_image' => 'nullable|array|max:5',
...
//            $additional_documents = Helpers::upload(dir:'additional_documents/dm/', format:$request->file('additional_documents')->getClientOriginalExtension(),  image:$request->file('additional_documents'));
//        } else {
//            $additional_documents = 'def.png';
//        }
    ...
    public function edit($id)
    {
        ...
        if (!empty($delivery_man['additional_documents'])) {
            $path = 'public/additional_documents/dm/' . $delivery_man['additional_documents'];
        ...
        $page_data = DataSetting::where('type', 'deliveryman')
            ->where('key', 'deliveryman_page_data')
            ->first()?->value;
        $page_data = $page_data ? json_decode($page_data, true) : [];
        $additional_data = json_decode($delivery_man->additional_data ?? '[]', true);

        return view('admin-views.delivery-man.edit', compact('delivery_man', 'page_data', 'additional_data', 'additional_documents_size'));
    }
...
//            $additional_documents = Helpers::upload(dir:'additional_documents/dm/', format:$request->file('additional_documents')->getClientOriginalExtension(),  image:$request->file('additional_documents'));
//        } else {
//            $additional_documents = $delivery_man['additional_documents'];
//        }

        // Build existing identity images from DB
        $existingImages = [];
        $rawImages = json_decode($delivery_man['identity_image'] ?? '[]', true);
        if (is_array($rawImages)) {
            foreach ($rawImages as $item) {
                if (is_array($item) && isset($item['img'])) {
                    $existingImages[] = $item;
                } elseif (is_string($item) && $item !== '') {
                    $existingImages[] = ['img' => $item, 'storage' => Helpers::getDisk()];
                }
            }
        }
        ...
        if ($request->has('deleted_images') && is_array($request->deleted_images)) {
            foreach ($request->deleted_images as $deleted) {
                if (!empty($deleted)) {
                    Helpers::check_and_delete('delivery-man/', $deleted);
                    $existingImages = array_values(array_filter($existingImages, function ($im) use ($deleted) {
                        return (is_array($im) ? ($im['img'] ?? null) : null) !== $deleted;
                    }));
                }
            }
        }

        // Enforce max limit: existing after deletions + new uploads <= 6
        $newFilesCount = 0;
        if ($request->hasFile('identity_image')) {
            foreach ($request->file('identity_image') as $f) {
                if ($f) { $newFilesCount++; }
            }
        }
        if ((count($existingImages) + $newFilesCount) > 6) {
            $errors = [];
            $errors = Helpers::error_formater('identity_image', 'You can upload a maximum of 6 identity images.', $errors);
            return response()->json(['errors' => $errors]);
        }

        // Append newly uploaded identity images (if any)
        if ($request->hasFile('identity_image')) {
            foreach ($request->file('identity_image') as $imgFile) {
                if ($imgFile) {
                    $uploaded = Helpers::upload(dir: 'delivery-man/', format: 'png', image: $imgFile);
                    if ($uploaded) {
                        $existingImages[] = ['img' => $uploaded, 'storage' => Helpers::getDisk()];
                    }
                }
            }
        }

        // Persist final identity images array
        $identity_image = !empty($existingImages) ? json_encode($existingImages) : json_encode([]);
        ...
        if(isset($request->additional_data)  && count($request->additional_data) > 0){
            $delivery_man->additional_data = json_encode($request->additional_data) ;
        }

        $additional_documents = [];
        if ($request->additional_documents) {

            foreach (json_decode($delivery_man['additional_documents'], true) as $img) {
                Helpers::check_and_delete('additional_documents/dm/' , $img);
            }
            foreach ($request->additional_documents as $key => $data) {
                $additional = [];
                foreach($data as $file){
            ...
            $delivery_man->additional_documents = json_encode($additional_documents);
        }
    ...
    public function delete(Request $request)
    {
        ...
        $decoded = json_decode($delivery_man['identity_image'] ?? '[]', true) ?: [];
        foreach ($decoded as $item) {
            $filename = is_array($item) ? ($item['img'] ?? null) : (is_string($item) ? $item : null);
            if ($filename) {
                Helpers::check_and_delete('delivery-man/' , $filename);
            }
        }
